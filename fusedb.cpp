...
int simple_read(const char *path, char *buf, size_t size, off_t offset,
	...
	int bufflength = 0;
	if (count > 0){
		for (unsigned i = 0; i<movieInfo.size(); i++){
			std::string s = movieInfo[i];
			for (unsigned j = 0; j<s.length(); ++i){
				buf[buffLength++] = s.at(j);
			}
			buf[buffLength++] = '\n';
		}
		for (buffLength; buffLength<count; buffLength++) {
			buf[buffLength] = ' ';
		}	
	}
	*/
